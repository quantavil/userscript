...
export class Crawler {
    ...
    private getSections(): HTMLElement[] {
        const selectors = [
            ...
            '#sectionNavTabs > li',
            '.sections-list > li',
            '.newInterface-sections .section-box'
        ];
        return Array.from(
    ...
    private getActiveSectionName(): string | null {
        ...
        const activeSec = sections.find(sec => 
            ...
            sec.classList.contains('section-box-current') || 
    ...
    private findNextButton(): HTMLElement | null {
        ...
        for (const el of els) {
            ...
            if (el.classList.contains('ng-hide') || cs.display === 'none' || cs.visibility === 'hidden') {
                continue;
            }
        ...
        return container.querySelector('.next-btn:not(.ng-hide), .btn-next:not(.ng-hide), [ng-click*="nextQuestion"]:not(.ng-hide)') as HTMLElement;
    }
}