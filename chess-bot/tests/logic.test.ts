...
import { uciToSan } from '../src/engine/san.js';
...
describe('Chess Logic Utils', () => {
    ...
    describe('uciToSan', () => {
        ...
        it('should handle standard UCI string moves', () => {
            expect(uciToSan(startFen, 'e2e4')).toBe('e4');
            expect(uciToSan(startFen, 'g1f3')).toBe('Nf3');
        });

        it('should handle UCI moves as objects', () => {
            expect(uciToSan(startFen, { uci: 'e2e4' })).toBe('e4');
            expect(uciToSan(startFen, { from: 'g1', to: 'f3' })).toBe('Nf3');

            const promoFen = '8/4P3/8/8/8/8/8/k3K3 w - - 0 1';
            expect(uciToSan(promoFen, { from: 'e7', to: 'e8', promo: 'q' })).toBe('e8=Q');
        });

        it('should handle invalid or short inputs gracefully', () => {
            expect(uciToSan(startFen, null)).toBe('');
            expect(uciToSan(startFen, undefined)).toBe('');
            expect(uciToSan(startFen, 'e2')).toBe('e2');
            expect(uciToSan(startFen, { from: 'e2' })).toEqual({ from: 'e2' });
        });
    });
