...
interface UI {
    menuWrap: HTMLDivElement | null;
    consoleEl: HTMLDivElement | null;
    lastStatus: string;
    lastBestMove: string;
    moveNumber: number;
    lastMoveColor: string | null;
    log(msg: string, type?: 'info' | 'status' | 'error' | 'warn' | 'move' | 'opponent'): void;
    clearConsole(): void;
    updateDisplay(playingAs: number): void;
...
export const ui: UI = {
  ...
  log(msg: string, type: 'info' | 'status' | 'error' | 'warn' | 'move' | 'opponent' = 'info'): void {
    if (!this.consoleEl && this.menuWrap) {
      this.consoleEl = this.menuWrap.querySelector('#consoleWindow') as HTMLDivElement | null;
    }
  ...
  updateDisplay(playingAs: number): void {
    ...
    // Only log if best move changed significantly
...
`;
  ...
  const getElementByName = (name: string, el: HTMLElement) => el.querySelector(`[name="${name}"]`) as HTMLElement | null;
  const getInputElement = (el: HTMLElement) => el.children[0] as HTMLInputElement;
  const getStateElement = (el: HTMLElement) => el.children[el.children.length - 1] as HTMLElement;

  function bindControl(name: string, type: 'checkbox' | 'range', variable: string) {
    ...
    const key = variable.replace('BotState.', '') as keyof typeof BotState;
    if (type === 'checkbox') {
      ...
      modInput.addEventListener('input', () => {
        if (name === 'moveMethod') {
          (BotState as any)[key] = modInput.checked ? 'drag' : 'click';
          modState.textContent = BotState[key] === 'drag' ? 'On' : 'Off';
        } else {
          (BotState as any)[key] = modInput.checked ? 1 : 0;
          modState.textContent = BotState[key] ? 'On' : 'Off';
        }
        Settings.save();
      });
      ...
      const toLog = (v: number) => Math.round(100 * Math.pow(100, v / 100)); // 100 * (100^x)
      const toLin = (v: number) => Math.log(v / 100) / Math.log(100) * 100;

      if (name === 'moveTime') {
        modInput.value = String(toLin(BotState.moveTime));
        ...
        const toLogJitter = (v: number) => {
          if (v === 0) return 0;
          return Math.round(100 * Math.pow(100, v / 100));
        };
        const toLinJitter = (v: number) => {
          if (v <= 0) return 0;
          return Math.log(v / 100) / Math.log(100) * 100;
        };

        modInput.value = String(toLinJitter(BotState.jitter));
        ...
        modInput.value = String(BotState[key]);
        modState.textContent = String(BotState[key]);
        modInput.addEventListener('input', () => {
          let value = parseInt(modInput.value, 10);
          const min = parseInt(modInput.min, 10);
          const max = parseInt(modInput.max, 10);
          value = Math.max(min, Math.min(max, value));
          (BotState as any)[key] = value;
          modInput.value = String(value);
          modState.textContent = String(value);
          Settings.save();
        });
      }
    }
  ...
  document.getElementById('consoleCopyBtn')!.addEventListener('click', () => {
    const consoleEl = menuWrap.querySelector('#consoleWindow') as HTMLDivElement | null;
    if (!consoleEl) return;
    const text = Array.from(consoleEl.querySelectorAll('.log-line'))
      .map(l => l.textContent!.trim())
      .join('\n');
    navigator.clipboard.writeText(text).then(() => {
      const btn = document.getElementById('consoleCopyBtn') as HTMLButtonElement | null;
      if (!btn) return;
...
function makePanelDraggable(panel: HTMLDivElement): void {
  function clampToViewport() {
    ...
    let left = parseFloat(panel.style.left || String(rect.left));
    let top = parseFloat(panel.style.top || String(rect.top));
  ...
  function allowDragFromTarget(target: Element, e: MouseEvent) {
  ...
  panel.addEventListener('mousedown', (e) => {
    ...
    if (!allowDragFromTarget(e.target as Element, e)) return;
    startDrag(e);
  });
