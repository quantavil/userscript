...
export interface EvalBuffers {
    wPawnFiles: Uint8Array;
    bPawnFiles: Uint8Array;
    wPawnRanks: Int8Array;
    bPawnRanks: Int8Array;
    bPawnMaxRanks: Int8Array;
    wPawnMinRanks: Int8Array;
    rookSquares: Int8Array;
    rookSides: Int8Array;
}
...
export interface LocalEngine {
    clearTT(): void;
    evaluate(): number;
    see(move: Move): number;
    scoreMoves(moves: Move[], ply: number, ttMove: Move | null): Int32Array;
    pickMove(moves: Move[], scores: Int32Array, startIdx: number): void;
    ttKey(): string;
    ttIndex(): number;
    ttProbe(depth: number, alpha: number, beta: number): { score: number | null; move: Move | null } | null;
    ttStore(depth: number, score: number, flag: number, move: Move | null): void;
    quiesce(alpha: number, beta: number, ply: number, qply?: number): number;
    negamax(depth: number, alpha: number, beta: number, ply: number, pvLine: Move[], ext: number): number;
    searchRoot(maxDepth: number, timeLimitMs: number): { move: Move | null; score: number; pv: Move[]; depth: number; nodes: number };
    analyze(fen: string, depth?: number, timeLimit?: number): { success: boolean; bestmove: string; evaluation: number; depth: number; nodes?: number; analysis?: any[]; source?: string; dirtyPlayTimeScale?: number };
}
...
export class LocalEngine {
    ...
    killers: number[][];
    history: Int32Array;
    tt: (TTEntry | undefined)[];
    evalBufs: EvalBuffers;
    hash: [number, number];
    phase: number;
    positionHistory: string[];
    contempt: number;
    rootSide: number;
    searchCompletedDepth: number;
    ...
    _unpackPieceCounts(packed: number): void {
    ...
    loadFen(fen: string): void {
        ...
        const pieceMap: Record<string, number> = { p: BP, n: BN, b: BB, r: BR, q: BQ, k: BK, P: WP, N: WN, B: WB, R: WR, Q: WQ, K: WK };
    ...
    _addPieceScore(p: number, sq: number): void {
        const side = p > 0 ? 1 : -1;
        const abs = p > 0 ? p : -p;
    ...
    _removePieceScore(p: number, sq: number): void {
        const side = p > 0 ? 1 : -1;
        const abs = p > 0 ? p : -p;
        const pstSq = side === 1 ? sq : (sq ^ 0x70);
    ...
    createMove(from: number, to: number, flags: number = FLAG_NONE, promo: number = EMPTY): Move {
        return {
            from, to, flags,
    ...
    generateMoves(capturesOnly: boolean = false): Move[] {
        const moves: Move[] = [];
        const s = this.side;
        const opp = -s;
        const bd = this.board;
    ...
    generateLegalMoves(capturesOnly: boolean = false): Move[] {
    ...
    getSmallestAttacker(sq: number, bySide: number): { piece: number; sq: number; value: number } | null {
        // Pawn attacks
        const pawnDir = bySide === 1 ? 1 : -1;
        const attackSourceRank = (sq >> 4) - pawnDir;
        if (attackSourceRank >= 0 && attackSourceRank <= 7) {
