...
export const SearchMethods = {
    // ---- Evaluation ----
    clearTT(this: LocalEngine): void {
        for (let i = 0; i < this.tt.length; i++) {
            this.tt[i] = undefined;
        }
    },

    evaluate(this: LocalEngine): number {
    ...
    see(this: LocalEngine, move: Move): number {
        ...
        // Simulate
        const removed: { sq: number; p: number }[] = [];
    ...
    // ---- Search ----
    scoreMoves(this: LocalEngine, moves: Move[], ply: number, ttMove: Move | null): Int32Array {
    ...
    pickMove(this: LocalEngine, moves: Move[], scores: Int32Array, startIdx: number): void {
    ...
    ttKey(this: LocalEngine): string {
        // String key for repetition detection history
        return this.hash[0] + '|' + this.hash[1];
    },

    ttIndex(this: LocalEngine): number {
        return this.hash[1] & 0xFFFF; // 65536 entries
    },

    ttProbe(this: LocalEngine, depth: number, alpha: number, beta: number): { score: number | null; move: Move | null } | null {
    ...
    ttStore(this: LocalEngine, depth: number, score: number, flag: number, move: Move | null): void {
        const index = this.ttIndex();
    ...
    quiesce(this: LocalEngine, alpha: number, beta: number, ply: number, qply = 0): number {
        this.nodes++;
    ...
    negamax(this: LocalEngine, depth: number, alpha: number, beta: number, ply: number, pvLine: Move[], ext: number): number {
        this.nodes++;
        if (this.stopped) return 0;
        if (this.nodes % 4096 === 0 && this.searchCompletedDepth >= 3 &&
            performance.now() - this.startTime > this.timeLimit) {
    ...
    searchRoot(this: LocalEngine, maxDepth: number, timeLimitMs: number): { move: Move | null; score: number; pv: Move[]; depth: number; nodes: number } {
        this.nodes = 0;
        ...
        for (let d = 1; d <= maxDepth; d++) {
            ...
            if (d > 1 && completedDepth > 0) {
                if (bestScore > 200) this.contempt = 60;
                else if (bestScore > 100) this.contempt = 45;
                else if (bestScore > 50) this.contempt = 35;
                else if (bestScore > -100) this.contempt = 25;
                else if (bestScore > -250) this.contempt = 10;
                else this.contempt = -15;
            } else {
                this.contempt = 25;
            }
    ...
    analyze(this: LocalEngine, fen: string, depth?: number, timeLimit?: number): any {
        this.loadFen(fen);
        this.clearTT();
