...
import { evaluatePremove, evaluatePremoveChain, getOurMoveFromPV, PremoveChainEntry } from './premove.js';
...
// Premove chain queue
let premoveChain: PremoveChainEntry[] = [];
let chainBaseFen = '';

export function getLastFenProcessedMain(): string { return lastFenProcessedMain; }
export function setLastFenProcessedMain(fen: string): void { lastFenProcessedMain = fen; }
export function getLastFenProcessedPremove(): string { return lastFenProcessedPremove; }
export function setLastFenProcessedPremove(fen: string): void { lastFenProcessedPremove = fen; }

/** Get the current premove chain (for external inspection) */
export function getPremoveChain(): PremoveChainEntry[] { return premoveChain; }

/** Clear the premove chain (e.g. on position divergence) */
export function clearPremoveChain(): void {
    premoveChain = [];
    chainBaseFen = '';
}
 ...
 */
export async function tryChainPremove(currentFen: string, tickCallback?: Function): Promise<boolean> {
    ...
    const result = evaluatePremove(currentFen, next.oppUci, next.uci, playerColor);
...
export function scheduleAnalysis(kind: 'main' | string, fen: string, tickCallback?: Function): void {
    // Dedup guard
    ...
    const run = async () => {
        ...
        try {
            ...
            if (kind === 'main') {
                ...
                const currentFen = getFen(getGame());
                if (currentFen) {
                ...
                if (!stm) {
                    BotState.statusInfo = BotState.premoveEnabled ? `Premove unavailable (no STM)` : `Pondering... (D${data.depth})`;
                    if (BotState.onUpdateDisplay) BotState.onUpdateDisplay(pa());
