...
export interface ZobristData {
  table: Uint32Array;
  sideKey: [number, number];
  castlingKeys: Uint32Array;
  epKeys: Uint32Array;
}

// Pre-computed random 64-bit keys stored as pairs of 32-bit ints for speed
export const ZOBRIST: ZobristData = (() => {
    ...
    const sideKey: [number, number] = [rand32(), rand32()];
...
// Zobrist helpers
export function zobPieceIdx(piece: number): number {
    // Map piece value (-6..-1, 1..6) to index 0-11. Empty=invalid
    return piece > 0 ? (piece - 1) : (-piece + 5);
}
export function zobPieceKey(piece: number, sq: number): [number, number] {
    const base = (zobPieceIdx(piece) * 128 + sq) * 2;
    return [ZOBRIST.table[base], ZOBRIST.table[base + 1]];
}
export function zobXor(hash: [number, number] | Uint32Array | number[], key: [number, number] | number[]): void {
