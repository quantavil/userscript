...
export function injectHeartUI(container: HTMLElement, meta?: WishlistItem) {
  ...
  const existingHeart = container.querySelector<HTMLDivElement>('.aiw-heart-container');
  if (existingHeart) {
    if (existingHeart.dataset.wishlistId === meta.id) {
      return;
    }
  ...
  window.addEventListener('click', (e) => {
    ...
    if (heartContainer) {
      ...
      if (currentActive) {
        ...
        if (getSettings().autocrawl && !meta.details) {
