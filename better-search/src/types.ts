...
/** CSS custom property → AccentDetails key, single source for apply/remove */
const ACCENT_VAR_MAP = [
    ['--svf-primary',           'primary'],
    ['--svf-primary-glow',      'glow'],
    ['--svf-primary-bg',        'bg'],
    ['--svf-primary-bg-hover',  'bgHover'],
    ['--svf-primary-border',    'border'],
    ['--svf-primary-status-bg', 'statusBg'],
] as const satisfies ReadonlyArray<readonly [string, keyof AccentDetails]>;
...
export function applyAccentVariables(el: HTMLElement, accent: AccentColor): void {
    ...
    for (const [varName, key] of ACCENT_VAR_MAP) {
        el.style.setProperty(varName, current[key]);
    }
}

export function removeAccentVariables(el: HTMLElement): void {
    for (const [varName] of ACCENT_VAR_MAP) {
