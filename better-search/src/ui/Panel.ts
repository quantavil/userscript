...
import { getRegisterableDomain } from '../utils/domain';
...
export class PanelElement extends LitElement {
    ...
    open(): void {
        if (this._isOpen) return; // guard: re-saving overflow would lock scrolling
    ...
    close(): void {
        ...
        if (card) {
            ...
            // Fallback: transitionend never fires when transitions are
            // suppressed (reduced motion, backgrounded tab)
            clearTimeout(this._hideTimeout);
            this._hideTimeout = window.setTimeout(hide, 400);
        } else {
            hide();
        }
    }
    ...
    /** Commit any list still in edit mode (called when closing or switching tabs). */
    private _autoSavePendingTextareaEdits(): void {
        for (const type of ['liked', 'disliked'] as const) {
            if (this._isTextareaEditable[type]) {
                ...
                this._saveDomains(type, this._getLines(textValue));
            }
        }
    ...
    private _renderFiltersTab() {
        return html`
            <div class="svf-search-wrapper">
                ...
                <input type="text" id="svf-unified-search" class="svf-search-input" placeholder="Filter all domains (e.g. github)..." .value=${this._searchQuery} @input=${(e: any) => this._searchQuery = e.target.value}>
    ...
    private _renderSection(type: 'liked' | 'disliked', titleText: string) {
        ...
        const matches = query ? domains.filter(d => d.includes(query)) : [];
        const badgeText = query ? `${matches.length} match${matches.length !== 1 ? 'es' : ''}` : String(domains.length);

        return html`
            <div class="svf-section ${type}">
                <div class="svf-section-header">
                    <div class="svf-label">${titleText}</div>
                    <span class="svf-badge">${badgeText}</span>
                </div>

                ${!query && proposalFound && activeProposal ? html`
                    <div class="svf-merger-card">
                        ...
                        <button class="svf-merger-btn" type="button" @click=${() => {
                            ...
                            this._saveDomains(type, filtered);
                ...
                ` : html`
                    ...
                    <div class="svf-textarea-status-row">
                        <div class="svf-textarea-msg-info">
                        ...
                        ${this._isTextareaEditable[type] ? html`
                            <div class="svf-status-actions" style="display: flex; gap: 8px;">
                                ...
                                <button class="svf-apply-btn" type="button" @click=${() => {
                                    this._saveDomains(type, lines);
                                    this._isTextareaEditable = { ...this._isTextareaEditable, [type]: false };
                                }}>
                                    Apply Changes
                                </button>
        ...
        return html`
            ...
            <div class="svf-control-group">
                ...
                <div class="svf-segmented">
                    <button class="svf-seg-btn ${mode === 'fade' ? 'active' : ''}" @click=${() => this.store.setDislikeMode('fade')}>Fade Results</button>
                    <button class="svf-seg-btn ${mode === 'hide' ? 'active' : ''}" @click=${() => this.store.setDislikeMode('hide')}>Hide (Reveal-on-click)</button>
                </div>
            </div>
            ...
            <div class="svf-control-group">
                <div class="svf-switch-row">
                    <div class="svf-switch-label-group">
                        <div class="svf-switch-title">Show settings floating button (FAB)</div>
                        <div class="svf-switch-desc">Toggle settings icon visibility in the bottom right</div>
                    </div>
                    <label class="svf-switch">
                        <input type="checkbox" .checked=${this.store.showTrigger} @change=${(e: any) => this.store.setShowTrigger(e.target.checked)}>
            ...
            <div class="svf-control-group">
                <div class="svf-switch-row">
                    <div class="svf-switch-label-group">
                        <div class="svf-switch-title">Show hover action buttons</div>
                        <div class="svf-switch-desc">Toggle quick action buttons (Star/Slash) when hovering search results on desktop</div>
                    </div>
                    <label class="svf-switch">
                        <input type="checkbox" .checked=${this.store.showHoverOverlay} @change=${(e: any) => this.store.setShowHoverOverlay(e.target.checked)}>
                        <span class="svf-slider"></span>
                    </label>
                </div>
            </div>
        ...
        return html`
            <div class="svf-io-section" style="border-top: none; padding-top: 0">
                ...
                <form class="svf-io-url-form" @submit=${(e: Event) => { e.preventDefault(); this._quickImportGist(); }}>
                    ...
                    <button type="submit" class="svf-io-btn primary" style="flex: 0 0 auto" ?disabled=${this._isFetchingUrl}>${this._isFetchingUrl ? 'Importing...' : 'Import'}</button>
                </form>

                ${this._ioStatus ? html`<div class="svf-io-status ${this._ioStatus.type}">${this._ioStatus.msg}</div>` : nothing}
            </div>
            ...
            <div class="svf-io-section svf-sync-section">
                ...
                ${this._syncStatus ? html`<div class="svf-io-status ${this._syncStatus.type}">${this._syncStatus.msg}</div>` : nothing}
            </div>
        `;
    }
    ...
    private _saveEdit(type: 'liked'|'disliked', oldMatch: string, newValRaw: string) {
        ...
        if (newVal && newVal !== oldMatch) {
            ...
            this._saveDomains(type, updated);
        }
    ...
    /** Persist a list; the store notifies subscribers (the Controller re-applies filters). */
    private _saveDomains(type: 'liked' | 'disliked', domains: string[]): void {
