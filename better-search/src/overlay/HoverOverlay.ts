...
export class HoverOverlayElement extends LitElement {
    ...
    connectedCallback() {
        ...
        if (this.store) {
            this._unsubscribe = this.store.subscribe(() => {
                ...
                if (!this.store.showHoverOverlay) {
                    this._scheduleHide();
                }
            });
        }
        
        // If the device has no hover capability, do not register pointer tracking listeners (save resources on mobile)
    ...
    private _onPointerMove(e: PointerEvent): void {
        ...
        if (!item) {
            ...
            if (target === host || this === target || this.shadowRoot?.contains(target)) {
                this._cancelHide();
            } else {
                this._scheduleHide();
            }
