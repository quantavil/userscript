...
const GLOBAL_KEY = '__svf_controller';
...
export class Controller {
    ...
    // Owns the SPA-navigation listeners. Separate from the page-scoped
    // AbortController so tearing down the page UI (e.g. after navigating to a
    // non-search page) doesn't also silence the events that would revive it.
    ...
    init(): void {
        const prevController = (window as any)[GLOBAL_KEY];
        ...
        window.addEventListener('popstate', this._onLocationChange, { signal: this._navAbort.signal });
        window.addEventListener('svf-location-change', this._onLocationChange, { signal: this._navAbort.signal });
        hookHistoryEvents();
    ...
    /** Build all page-scoped pieces for the currently detected engine. */
    private _setupPage(): void {
        this._teardownPage(); // clean teardown of any existing page state
        ...
        try {
            ...
            this._hoverOverlay.panel = this._panel;
            shadow.appendChild(this._hoverOverlay);

            this._mobileSwipe = new MobileSwipe(this._store, signal);
            ...
            if (typeof GM_registerMenuCommand === 'function') {
                ...
                GM_registerMenuCommand('Toggle Settings Button', () => {
                    if (this._store) {
                        this._store.setShowTrigger(!this._store.showTrigger);
                    }
                });
            }
    ...
    /** Tear down everything page-scoped; navigation listeners stay alive. */
    private _teardownPage(): void {
    ...
    private _onLocationChange(): void {
        ...
        this._locationTimeout = setTimeout(() => {
            ...
            if (!nextEngine) {
                this._teardownPage(); // keep nav listeners so we can revive later
                return;
            }

            // Only re-init if the engine type has actually changed (or we're currently torn down)
