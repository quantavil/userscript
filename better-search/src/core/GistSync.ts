...
import { mergeDomainData } from '../utils/merge';

// NOTE: push() does pull-union-then-PATCH without ETag/If-Match, so two
// devices writing inside the same debounce window can still lose an update.
// Acceptable for a personal sync; a proper fix would send If-Match with the
// gist's ETag and retry on 412.
...
export class GistSync {
    ...
    async pull(mode: 'replace' | 'union' = 'replace', isInternal = false): Promise<number> {
        ...
        try {
            ...
            if (mode === 'union') {
                const merged = mergeDomainData(local, {
                    liked: rLiked,
                    disliked: rDisliked,
                    timestamps: remote.timestamps || {},
                });
                liked = merged.liked;
                disliked = merged.disliked;
                mergedTimestamps = merged.timestamps;
            }
            ...
            // Union: report how many domains were actually added locally
            const localSet = new Set([...local.liked, ...local.disliked]);
            return [...liked, ...disliked].filter(d => !localSet.has(d)).length;
        } finally {
