...
import { getFullscreenContainer, preventPropagation, vibrate } from '../utils';
import { SpeedStepper } from './panels/SpeedStepper';
import { SettingsSheet } from './panels/SettingsSheet';
import { getSvgIcon, IconName } from './icons';
...
export class UIManager {
    ...
    private buildSideBar(prefix: 'volume' | 'brightness', container: HTMLElement) {
    ...
    private positionSideBar(bar: HTMLDivElement, side: 'left' | 'right') {
        ...
        const styles: Record<string, string> = { top: `${top}px`, height: `${barH}px` };
        if (side === 'right') {
    ...
    public showUI(force = false) {
        ...
        // Timeout fade guard
        const now = Date.now();
        if (!force && (now - this.store.lastRealUserEvent >= MVC_CONFIG.INTERACTION_TIMEOUT)) return;
        ...
        if (!isInteracting && !this.store.activeVideo.paused) {
            this.store.timers.hide = setTimeout(() => this.hideUI(), MVC_CONFIG.UI_FADE_TIMEOUT) as any;
        }
    }
