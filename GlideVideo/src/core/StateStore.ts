...
export class StateStore {
    ...
    get isPinching(): boolean { return this.gestureCoordinator.isActive('pinch'); }
    get isSwipeSeeking(): boolean { return this.gestureCoordinator.isActive('swipe_seek'); }
    get isVolumeControlling(): boolean { return this.gestureCoordinator.isActive('volume_control'); }
    get isDoubleTapping(): boolean { return this.gestureCoordinator.isActive('double_tap'); }
    get isBrightnessControlling(): boolean { return this.gestureCoordinator.isActive('brightness_control'); }
    ...
    private readonly videoMetadata = new WeakMap<HTMLVideoElement, VideoMetadata>();
    ...
    public updateVideoMetadata(video: HTMLVideoElement, updates: Partial<VideoMetadata>) {
    ...
    private getVideoId(v: HTMLVideoElement): string {
        ...
        if (src && !src.startsWith('blob:') && !src.startsWith('data:')) {
            ...
            if (!cleanPath) {
                ...
                const delimiter = url.includes('#') ? (url.endsWith('#') ? '' : '#') : '#';
...
function getVideoDomPath(v: HTMLVideoElement): string {
    ...
    while (current && current !== docBody) {
        ...
        const parent = current.parentElement || (parentNode && (parentNode as any).host ? (parentNode as any).host : null);
