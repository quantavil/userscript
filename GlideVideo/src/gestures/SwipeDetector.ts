	...
	private attachSwipeListeners() {
		window.addEventListener(
			...
			(e) => {
				...
				if (shouldBlockGestures() && !this.store.settings.scrollCompatibility)
					return;
				...
				if (e.touches.length === 2) {
					...
					if (
						!isPointInRect(t1.clientX, t1.clientY, this.store.activeVideo) ||
						!isPointInRect(t2.clientX, t2.clientY, this.store.activeVideo)
					)
						return;
				...
				if (
					touch.clientX < MVC_CONFIG.EDGE_TOUCH_PROTECTION_PADDING ||
					touch.clientX >
				...
				if (!this.store.activeVideo?.isConnected) return;
				if (
					!isPointInRect(touch.clientX, touch.clientY, this.store.activeVideo)
				)
					return;
				...
				onTouchMove = (ev: TouchEvent) => {
					if (this.store.savedPlaybackRate !== undefined || this.isPinching)
						return;
					...
					if (mode === "undecided") {
						const overThreshold =
							absDx > MVC_CONFIG.GESTURE_MOVE_THRESHOLD ||
							absDy > MVC_CONFIG.GESTURE_MOVE_THRESHOLD;
					...
					if (mode === "seek") {
						...
						if (
							...
							timeChange =
								timeChange -
								Math.sign(timeChange) * MVC_CONFIG.GESTURE_SEEK_DEADZONE;
						}
						if (timeChange === 0) {
					...
					if (mode === "volume") {
						...
						const sensitivity =
							1 /
							Math.max(vr.height * MVC_CONFIG.GESTURE_VERTICAL_SENSITIVITY, 1);
					...
					if (mode === "brightness") {
						...
						const sensitivity =
							1 /
							Math.max(vr.height * MVC_CONFIG.GESTURE_VERTICAL_SENSITIVITY, 1);
						const newBrightness = clamp(
							startBrightness - dy * sensitivity,
							0.1,
							1.0,
						);
						this.store.brightness = newBrightness;
						this.eventBus.emit("ui:brightness-changed", {
