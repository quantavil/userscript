...
export type MvcEvents = {
    ...
    'video:double-tap-skipped': { side: 'left' | 'right'; x: number; y: number; seconds: number };
    'ui:volume-changed': { volume: number };
    'ui:brightness-changed': { brightness: number };
};

export class EventBus {
    private listeners: { [K in keyof MvcEvents]?: Array<(payload: MvcEvents[K]) => void> } = {};

    public emit<K extends keyof MvcEvents>(event: K, payload: MvcEvents[K]): void {
    ...
    public on<K extends keyof MvcEvents>(event: K, cb: (payload: MvcEvents[K]) => void): () => void {
        if (!this.listeners[event]) {
            this.listeners[event] = [];
        }
        this.listeners[event]!.push(cb);
        return () => {
            this.listeners[event] = (this.listeners[event] as any)!.filter((x: any) => x !== cb);
        };
    }
}
