...
export class VideoTransform implements EventListenerObject {
	...
	public destroy() {
		...
		if (this.currentScrollParents) {
			this.currentScrollParents.forEach((p) =>
				p.removeEventListener("scroll", this.boundScrollHandler),
			);
		}
		if (this.lastVideo) {
	...
	private attachGlobalListeners() {
		...
		window.addEventListener(
			...
			() => {
				...
				}, MVC_CONFIG.SCROLL_END_TIMEOUT) as any;
	...
	private onActiveVideoChanged(v: HTMLVideoElement | null) {
		...
		if (v) {
			...
			scrollParents.forEach((p) =>
				p.addEventListener("scroll", this.boundScrollHandler, {
					passive: true,
				}),
			);
			...
			const savedRate =
				meta.lastRate !== undefined ? meta.lastRate : rememberedRate;
	...
	public _applyPagePosition(
		...
		if (this.ui.wrap.style.bottom !== "auto")
	...
	public positionOnVideo() {
		...
		const desiredLeftPage =
			vr.right - offsetX + window.scrollX - MVC_CONFIG.EDGE - uiWidth;
		const desiredTopPage = vr.top + offsetY + window.scrollY + MVC_CONFIG.EDGE;

		this._applyPagePosition(
			desiredLeftPage,
			desiredTopPage,
			this.store.isScrolling,
		);
	...
	public ensureUIInViewport() {
		if (
			!this.ui.wrap ||
			!this.ui.wrap.offsetWidth ||
			!this.ui.wrap.offsetHeight
		)
			return;
		const uiRect = this.ui.wrap.getBoundingClientRect();
		this._applyPagePosition(
			uiRect.left + window.scrollX,
			uiRect.top + window.scrollY,
		);
	}
