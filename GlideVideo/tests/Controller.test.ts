...
describe('Controller', () => {
    ...
    it('should initialize collapsible controls in UIManager and handle toggling and timers', () => {
        const controller = new Controller();
        controller.init();

        const ui = controller.ui;
        expect(ui.controlsRow).toBeDefined();
        expect(ui.collapseBtn).toBeDefined();

        const classListAddSpy = vi.spyOn(ui.controlsRow!.classList, 'add');
        const classListRemoveSpy = vi.spyOn(ui.controlsRow!.classList, 'remove');
        const classListContainsSpy = vi.spyOn(ui.controlsRow!.classList, 'contains');

        // Initially expanded/collapsed check
        classListContainsSpy.mockReturnValue(true); // mock contains('collapsed') to be true
        ui.toggleControlsRow();
        expect(classListRemoveSpy).toHaveBeenCalledWith('collapsed');

        classListContainsSpy.mockReturnValue(false); // mock contains('collapsed') to be false
        ui.toggleControlsRow();
        expect(classListAddSpy).toHaveBeenCalledWith('collapsed');
    });
});
