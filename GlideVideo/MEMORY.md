...
- **Persistent Video Zoom & Ratio**: Zoom/aspect ratio transforms persisted in `localStorage` and carried over between videos, causing newly launched videos to feel zoomed in by default. Fixed by resetting transform settings to default on new video loads and making them session-only/in-memory.
- **Swipe Seek Jitter/Sensitivity**: Swipe gestures committed to seeks too easily, causing minor accidental slides to register small playback shifts. Also, releasing the gesture would unconditionally seek to `newTime` (the original capture time if change was under threshold), causing the video to skip backward by the amount of time elapsed during the touch. Fixed by increasing the deadzone resistance threshold to 4s (offsetting smoothly by 3s to start at ±1s skip delta), hiding the gesture overlay when under threshold (so it doesn't show "+0s"), and guarding the `currentTime` assignment in `onTouchEnd` to only run if the playhead actually changed.
- **Unused LongPress Getters**: Dead getters `isInLongPressGesture()` and `isLongPressFired()` in `PressDetector.ts` were unused in production. Removed and exposed the state properties directly to tests.
- **Redundant Entry point Subclass**: The entry point in `index.ts` defined a redundant `MobileVideoController` class extending `Controller`. Removed and instantiated `Controller` directly.
- **X-Mode / Scroll Feed Compatibility**: Added portrait-mode feed scroll compatibility allowing horizontal seeks on videos while bypassing vertical swipes to let the parent page scroll.
- **Volume Boost (>100%)**: Integrated AudioContext and GainNode amplification up to 200% with orange boost overlay styles and graceful CORS warning fallback.
- **Gesture Screen Lock**: Added a padlock shield locking all gesture interactions and hiding other control buttons, retaining only the clickable unlock button.
- **Mock Audio Gain Connect Blunder**: Volume boost test threw type errors due to a missing mock `connect()` method on the mock gain node. Fixed by including `connect: vi.fn()`.
