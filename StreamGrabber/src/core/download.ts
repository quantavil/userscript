...
export async function downloadDirect(
  ...
  // Capture abort handle from GM_download
  let abortDownload: { abort(): boolean } | undefined;

  card.setOnCancel(() => {
    try { abortDownload?.abort(); } catch { /* some managers throw */ }
    cleanup();
    card.remove();
  });
  ...
  abortDownload = GM_download({
    ...
    onload: () => {
      card.update(100, '');
      card.done(true, '', dlUrl, info?.blob ?? undefined);
      cleanup();
