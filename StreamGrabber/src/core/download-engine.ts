...
export class SegmentDownloader {
  ...
  constructor(
    ...
    private onComplete: (success: boolean, message?: string, blob?: Blob) => void
  ) {
  ...
  private async finalize(): Promise<void> {
    ...
    try {
      if (success) {
        ...
        this.onComplete(true, undefined, blob);
      } else {
        this.writer.abort();
...
export async function downloadSegments(
  ...
  const downloader = new SegmentDownloader(
    ...
    (success, message, blob) => {
      if (success) {
        card.update(100, '');
        const downloadUrl = blob ? URL.createObjectURL(blob) : undefined;
        card.done(true, undefined, downloadUrl, blob);
      } else {
