...
function setupTopHandlers(): void {
  ...
  bus.on('SG_PROGRESS_DONE', (payload) => {
    const { id, ok, msg, blob } = payload as any;
    const card = remoteJobs.get(id);
    if (card) {
      const downloadUrl = (ok && blob) ? URL.createObjectURL(blob) : undefined;
      card.done(ok, msg, downloadUrl);
      // Retain card for 60 seconds to allow the user time to long-press and save
      setTimeout(() => remoteJobs.delete(id), 60000);
    }
  });
