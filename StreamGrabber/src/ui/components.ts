...
class ProgressCard implements ProgressCardController {
  ...
  update(percent: number, text = ''): void {
  ...
  done(ok = true, msg?: string, downloadUrl?: string, blob?: Blob): void {
    ...
    if (ok && downloadUrl) {
      // Create a prominent "Save File" download button
      const saveBtn = h('a', {
        class: 'sg-save-link',
        href: downloadUrl,
        download: this.title,
      ...
      // Click listener: on mobile, try to use GM_download with the native Blob object on tap.
      // Since it's inside a user-click handler, this runs in trusted event context.
      if (blob && typeof GM_download === 'function') {
        saveBtn.addEventListener('click', (e) => {
          ...
          GM_download({
            ...
            onerror: () => {
              // Fallback to direct navigation if GM_download fails
      ...
      // Auto-remove after 60 seconds (instead of 2.5s) to allow user time to interact/long-press
      setTimeout(() => this.remove(), 60000);
    } else {
